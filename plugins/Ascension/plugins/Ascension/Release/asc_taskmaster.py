...
def main():
    ...
    if task_action == "wipe":
        ...
        """
        action_desc = "deleted performer custom field history"
        display_name = "Wipe Performer History"
        ...
        """
        action_desc = "reset performer ratings to null"
        display_name = "Reset Performer Ratings"

    elif task_action == "scenewipe":
        scene_mutation = """
        mutation DeleteSceneFields($id: ID!) {
          sceneUpdate(input: {
            id: $id,
            custom_fields: { remove: ["hotornot_stats", "scene_record"] }
          }) { id }
        }
        """
        action_desc = "deleted scene custom field history"
        display_name = "Wipe Scene History"

    elif task_action == "scenereset":
        scene_mutation = """
        mutation ResetSceneRatings($id: ID!) {
          sceneUpdate(input: {
            id: $id,
            rating100: null
          }) { id }
        ...
        """
        action_desc = "reset scene ratings to null"
        display_name = "Reset Scene Ratings"
        ...
        # Fetch scene ratings and custom_fields
        scene_query = """
        query GetAllScenes {
        ...
        try:
            ...
            for scene in scenes_details:
                export_data["scenes"].append({
                    ...
                    "stats": scene.get("custom_fields", {}).get("hotornot_stats"),
                    "record": scene.get("custom_fields", {}).get("scene_record")
                })
        ...
        try:
            ...
            scene_custom_fields_mutation = """
            mutation ImportSceneCustomFields($id: ID!, $custom_fields: CustomFieldsInput!) {
              sceneUpdate(input: {
                id: $id,
                custom_fields: $custom_fields
              }) { id }
            ...
            for performer_data in performers_data:
                ...
                if performer_id:
                    ...
                    if performer_data.get("rating") is not None:
                        rating_variables = {
                            "id": performer_id,
                    ...
                    custom_fields_data = {}
                    if performer_data.get("stats") is not None:
                        custom_fields_data["hotornot_stats"] = performer_data["stats"]
                    if performer_data.get("record") is not None:
            ...
            # Import scene ratings and custom fields
            ...
            for scene_data in scenes_data:
                ...
                scene_rating = scene_data.get("rating")
                scene_stats = scene_data.get("stats")
                scene_record = scene_data.get("record")
                scene_updated = False
                ...
                scene_custom_fields_data = {}
                if scene_stats is not None:
                    scene_custom_fields_data["hotornot_stats"] = scene_stats
                if scene_record is not None:
                    scene_custom_fields_data["scene_record"] = scene_record

                if scene_custom_fields_data:
                    scene_custom_fields_variables = {
                        "id": scene_id,
                        "custom_fields": {
                            "partial": scene_custom_fields_data
                        }
                    }

                    res = call_graphql(STASH_URL, scene_custom_fields_mutation, scene_custom_fields_variables, cookies=cookies)

                    if res and "errors" in res:
                        error_detail = f"[DEBUG] GraphQL Error importing custom fields for scene {scene_id}: {res['errors']}"
                        if STASHAPI_AVAILABLE:
                            log.debug(error_detail)
                        else:
                            print(error_detail, file=sys.stderr)
    ...
    if task_action in ["scenewipe", "scenereset"]:
        if STASHAPI_AVAILABLE:
            log.info(f"Starting {display_name} task...")

        find_scenes_query = "{ findScenes(filter: { per_page: -1 }) { scenes { id } } }"
        scene_result = call_graphql(STASH_URL, find_scenes_query, cookies=cookies)

        if not scene_result or "data" not in scene_result or not scene_result["data"]:
            error_msg = "[ERROR] Could not fetch scenes."
            if STASHAPI_AVAILABLE:
                log.error(error_msg)
            else:
                print(error_msg, file=sys.stderr)
        ...
        if "findScenes" not in scene_result["data"] or not scene_result["data"]["findScenes"]:
            error_msg = "[ERROR] No scenes found in response."
            if STASHAPI_AVAILABLE:
                log.error(error_msg)
            else:
                print(error_msg, file=sys.stderr)
        ...
        scenes = scene_result["data"]["findScenes"]["scenes"]
        total = len(scenes)
        success_count = 0

        for idx, s in enumerate(scenes):
            sid = s["id"]
            res = call_graphql(STASH_URL, scene_mutation, {"id": sid}, cookies=cookies)

            if res and "errors" in res:
                error_detail = f"[DEBUG] GraphQL Error on scene ID {sid}: {res['errors']}"
                if STASHAPI_AVAILABLE:
                    log.debug(error_detail)
                else:
                    print(error_detail, file=sys.stderr)
    ...
    elif task_action not in ["snapshotexport", "snapshotimport"]:
        if STASHAPI_AVAILABLE:
            log.info(f"Starting {display_name} task...")
    ...
    if task_action in ["scenewipe", "scenereset"]:
        output_msg = f"Successfully {action_desc} for {success_count} scenes."
    elif task_action == "snapshotimport":
        output_msg = f"Successfully {action_desc}."
    else:
